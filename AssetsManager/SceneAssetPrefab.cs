		...
		public void LoadAsset(){
			StartCoroutine (SubBuild_AssetBundleManager.LoadAsset (
				(myReturnValue) => {
					...
					#if ClientCode
					Shader Mobile = Shader.Find ("Mobile/Diffuse");
					Shader HighQuality = Shader.Find ("Standard");
					Shader Cartoon = Shader.Find ("Toon/Lit Outline");
						...
						foreach (MeshParameter meshParameter in meshParameters) {
							...
							if(TargetRenderer){
								...
								for(int i=0;i<TargetRenderer.sharedMaterials.Length;i++){
									...
									if(TargetRenderer.sharedMaterials[i].shader.name == HighQuality.name||TargetRenderer.sharedMaterials[i].shader.name == Mobile.name){
										if (PlayerPrefs.GetInt ("AdvanceShader") == 0) {
											TargetRenderer.sharedMaterials[i].shader = Mobile;
										}else if(PlayerPrefs.GetInt ("AdvanceShader") == 1){
											TargetRenderer.sharedMaterials[i].shader = HighQuality;
										}else if(PlayerPrefs.GetInt ("AdvanceShader") == 2){
											TargetRenderer.sharedMaterials[i].shader = Cartoon;
											TargetRenderer.sharedMaterials[i].SetFloat ("_Outline", 0.03f);
											TargetRenderer.sharedMaterials[i].SetColor ("_OutlineColor", Color.black);
											TargetRenderer.sharedMaterials[i].SetTexture("_Ramp",AssetBundleManager.CartoonTexture);
										}
									}else {
